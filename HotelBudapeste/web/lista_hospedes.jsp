...
<%@ page import="modelo.Hospede, modelo.HospedeVip, java.util.List" %>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Lista de Hóspedes - Hotel Budapeste</title>
...
<%
    ...
    List<Hospede> lista = (List<Hospede>) request.getAttribute("lista");
%>
    ...
    <div class="w-full max-w-6xl mx-auto glass-card p-6 sm:p-10 rounded-xl">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-3xl font-extrabold text-white drop-shadow-lg">🏨 Lista de Hóspedes</h2>
            <a href="hospede.jsp" class="text-base font-bold text-white bg-white/10 hover:bg-white/20 border border-white/20 transition duration-150 py-2 px-6 rounded-lg shadow-md backdrop-blur-md">
                + Novo Hóspede
            </a>
        </div>
                ...
                <table class="min-w-full divide-y divide-white/10">
                    <thead>
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider">Nome Completo</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider">CPF</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider">Data Nasc.</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider">Email</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider">VIP</th> 
                    ...
                    <tbody class="divide-y divide-white/10">
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-4 whitespace-nowrap text-sm"><%= h.getNome() %> <%= h.getSobrenome() %></td>
                            <td class="px-4 py-4 whitespace-nowrap text-sm"><%= h.getCpf() %></td>
                            <td class="px-4 py-4 whitespace-nowrap text-sm"><%= h.getDataNascimento() %></td>
                            <td class="px-4 py-4 whitespace-nowrap text-sm"><%= h.getEmail() != null ? h.getEmail() : "-" %></td>
                            <td class="px-4 py-4 whitespace-nowrap text-sm">
                                <span class="<%= (h instanceof HospedeVip) ? "text-green-400 font-bold" : "text-white/40" %>">
                                    <%= (h instanceof HospedeVip) ? "Sim" : "Não" %>
                                </span>
                            </td> 
                            <td class="px-4 py-4 whitespace-nowrap text-sm font-medium flex justify-center items-center gap-4">
                                <a href="hospede?acao=edit&id=<%= h.getId() %>" class="text-yellow-400 hover:text-yellow-200 transition">Editar</a>
                                <form action="hospede" method="post" onsubmit="return confirm('Excluir hóspede <%= h.getNome() %>?')">
        ...
        <% } else { %>
            <div class="p-12 text-center border-2 border-dashed border-white/20 rounded-lg bg-white/5">
                <p class="text-lg text-white/60 font-semibold">Nenhum hóspede cadastrado.</p>
            </div>
        <% } %>
