...
<%@ page import="modelo.Quarto, java.util.List" %>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Lista de Quartos - Hotel Budapeste</title>
...
<%
    ...
    List<Quarto> lista = (List<Quarto>) request.getAttribute("lista");
%>
    ...
    <div class="w-full max-w-6xl mx-auto glass-card p-6 sm:p-10 rounded-xl">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-3xl font-extrabold text-white drop-shadow-lg">🛏️ Lista de Quartos</h2>
            <a href="quarto.jsp" class="text-base font-bold text-white bg-white/10 hover:bg-white/20 border border-white/20 transition duration-150 py-2 px-6 rounded-lg shadow-md backdrop-blur-md">
                + Novo Quarto
            </a>
        </div>
                ...
                <table class="min-w-full divide-y divide-white/10">
                    <thead>
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider">Número</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider">Tipo</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-center">Andar</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider">Diária</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider">Status</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider">Recursos</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-center">Ações</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-white/10">
                        <% for (Quarto q : lista) { 
                            String statusStyle = "text-white/40";
                            if ("Disponível".equalsIgnoreCase(q.getStatus())) statusStyle = "text-green-400 font-bold";
                            else if ("Ocupado".equalsIgnoreCase(q.getStatus())) statusStyle = "text-blue-400 font-bold";
                            else if ("Manutenção".equalsIgnoreCase(q.getStatus())) statusStyle = "text-red-400 font-bold";
                        %>
                        <tr>
                            <td class="px-4 py-4 whitespace-nowrap text-sm font-medium"><%= q.getId() %></td>
                            <td class="px-4 py-4 whitespace-nowrap text-sm font-bold text-white"><%= q.getNumero() %></td>
                            <td class="px-4 py-4 whitespace-nowrap text-sm"><%= q.getTipo() %></td>
                            <td class="px-4 py-4 whitespace-nowrap text-sm text-center"><%= q.getAndar() %>º</td>
                            <td class="px-4 py-4 whitespace-nowrap text-sm font-semibold">R$ <%= String.format("%.2f", q.getValorDiaria()) %></td>
                            <td class="px-4 py-4 whitespace-nowrap text-sm">
                                <span class="<%= statusStyle %>"><%= q.getStatus() %></span>
                            </td> 
                            <td class="px-4 py-4 whitespace-nowrap text-xs">
                                <%= q.isWifiDisponivel() ? "📶 WiFi " : "" %><%= q.isTemCamaExtra() ? "🛏️ Cama" : "" %>
                            </td>
                            <td class="px-4 py-4 whitespace-nowrap text-sm font-medium flex justify-center items-center gap-4">
                                <a href="quarto?acao=edit&id=<%= q.getId() %>" class="text-yellow-400 hover:text-yellow-200 transition">Editar</a>
                                <form action="quarto" method="post" onsubmit="return confirm('Excluir quarto <%= q.getNumero() %>?')">
        ...
        <% } else { %>
            <div class="p-12 text-center border-2 border-dashed border-white/20 rounded-lg bg-white/5">
                <p class="text-lg text-white/60 font-semibold">Nenhum quarto cadastrado.</p>
                <a href="quarto.jsp" class="text-blue-400 hover:text-blue-300 text-sm mt-2 block">Cadastre o primeiro agora</a>
            </div>
        <% } %>
